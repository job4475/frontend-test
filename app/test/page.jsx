...
const Sidebar = () => {
  ...
  return (
    <div className='flex bg-[#f8fafc]'>
      <div className={`bg-[#fff] h-screen px-4 py-6 relative duration-100 ${open ? "w-64" : "w-20"}`}>
          ...
          {Menus.map((menu, index) => (
            <>
              <li key={index} className='text-[#1F2939] rounded-md text-sm font-medium flex items-center gap-x-3 cursor-pointer px-2 py-2.5 mt-1 hover:bg-[#1F2939] hover:text-[#fff] has-[:checked]:bg-indigo-50'>
                <span sx={{ fontSize: '25px' }} className='block float-left'>
      ...
      <div className='bg-[#fff] w-screen h-16 flex items-center px-7'>
        <div className='text-xl text-[#1F2939] font-semibold'>Home</div>
      </div>
    </div>
  )
}
