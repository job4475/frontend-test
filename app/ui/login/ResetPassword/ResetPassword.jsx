...
function Index() {
  ...
  useEffect(() => {
    if (expirationTime < currentTime) {
      setState((prevData) => ({ ...prevData, alert: true, alert_text: "Token expired", alert_type: "error" }));
      router.push('/Login');
    } else {
      setState({ ...state, checktoken: true, confirmlink: tokenParam, confirmlink_decode: decodedToken, email: decodedToken.username, });
    }
  }, [expirationTime, currentTime, tokenParam]);
      ...
      <Box p={3} sx={{ display: 'flex', flexDirection: 'column', width: '400px', height: '500px', borderRadius: "15px", marginLeft: 'auto', marginRight: 5, marginTop: 1, boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)', justifyContent: 'space-between' }}>
        <Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginTop: 2 }}>
            ...
            {state.checktoken ? (
              <TextField label="Confirm Password" value={state.confirmPassword} variant="standard" type={showPassword ? 'text' : 'password'} onChange={confirmPassword} sx={{ width: '90%' }} InputProps={{
                endAdornment: (<IconButton onClick={Confirm} edge="end">
          ...
          {state.checktoken ? (
            <Box sx={{ display: 'flex', mt: "18px", width: '90%', ml: 'auto', mr: 'auto' }}>
              <Box sx={{ bgcolor: state.passwordStrength === "Very Weak" ? '#E8E8E8' : '#2AB930', width: '20%', height: '5px', mr: "2px" }}></Box>
        ...
        {state.checktoken ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <Button
              ...
              style={{
                width: '90%', height: '44px', textTransform: 'capitalize', marginTop: 10, boxShadow: '0px 0px 0px', borderRadius: '8px',
