    ...
    return (
        <CustomBackground>
            ...
            {showContent ? (<Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                <TextField id="standard-basic" label="Email" variant="standard" value={state.email} onChange={HandleLogin.Email} onKeyDown={handleEnterKeyPress} sx={{ width: '100%', '& label': { color: '#828895', fontWeight: '500' } }} />
                {state.info === 0 && (
                    <TextField label="Password" variant="standard" value={state.password} onChange={HandleLogin.Password} onKeyDown={handleEnterKeyPress} type={state.showPassword ? 'text' : 'password'} sx={{ width: '100%', mt: '15px', '& label': { color: '#828895', fontWeight: '500' } }}
                        InputProps={{
                            endAdornment: (
                ...
                {state.info === 0 && (
                    <Box sx={{ background: '', width: '100%', display: 'flex', justifyContent: 'end', mt: '8px' }}>
                        <Box variant="text" onClick={HandleLogin.ForgotPassword} sx={{ fontSize:'14px',fontWeight:'500' , color: '#828895', textTransform: 'capitalize', '&:hover': { background: 'none', color: '#1F2939', cursor:'pointer'}, '&:active': { background: 'none' } }}>Forgot Password?</Box>
            ...
            {showContent ? (
                <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                    <Button variant="contained" onClick={onClickHandler} sx={{ transition: 'transform 0.3s ease', '&:hover': { transform: 'scale(1.03)', }, }} style={{ background: state.loading ? '#e5e5e5' : '#84BAA1', width: '100%', height: '44px', textTransform: 'capitalize', marginTop: 10, boxShadow: '0px 0px 0px', borderRadius: '8px', fontWeight: '600', cursor: state.loading ? 'not-allowed' : 'pointer', }} disabled={state.loading}>
                        {buttonText}
                    </Button>
                    {state.open ? <Dialog /> : ""}
                </Box>
